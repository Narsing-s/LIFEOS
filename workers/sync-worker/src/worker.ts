...
async function refreshGoogle(connection: Connection) {
  ...
  const { encryptSecret } = await import('../../apps/api/src/services/crypto.js');
...
async function googleJson(path:string, token:string) {
...
async function syncGoogle(connection: Connection, runId:string) {
  ...
  const now = new Date();
  try {
    const calendar = await googleJson('calendar/v3/calendars/primary/events?maxResults=50&singleEvents=true&orderBy=startTime', token);
    for (const event of calendar.items ?? []) {
      if (!event.id || !event.summary) continue;
      await db.$executeRaw(Prisma.sql`INSERT INTO timeline_events(user_id,event_type,title,description,occurred_at,end_at,source_provider,source_id,metadata) VALUES(${connection.user_id},'CALENDAR',${event.summary},${event.description ?? null},${new Date(event.start?.dateTime ?? event.start?.date ?? now)},${event.end?.dateTime ? new Date(event.end.dateTime) : null},'GOOGLE',${event.id},${JSON.stringify({calendarId:'primary',htmlLink:event.htmlLink ?? null})}::jsonb) ON CONFLICT DO NOTHING`);
      imported++;
    }
  } catch (e) { errors.push(`calendar: ${(e as Error).message}`); }
  try {
    const drive = await googleJson("drive/v3/files?pageSize=50&orderBy=modifiedTime%20desc&fields=files(id,name,mimeType,modifiedTime,webViewLink)", token);
    for (const file of drive.files ?? []) {
      if (!file.id || !file.name) continue;
      await db.$executeRaw(Prisma.sql`INSERT INTO timeline_events(user_id,event_type,title,occurred_at,source_provider,source_id,metadata) VALUES(${connection.user_id},'DRIVE_FILE',${file.name},${new Date(file.modifiedTime ?? now)},'GOOGLE',${file.id},${JSON.stringify({mimeType:file.mimeType,webViewLink:file.webViewLink ?? null})}::jsonb) ON CONFLICT DO NOTHING`);
      imported++;
    }
  } catch (e) { errors.push(`drive: ${(e as Error).message}`); }
  try {
    const gmail = await googleJson('gmail/v1/users/me/messages?maxResults=25', token);
    for (const message of gmail.messages ?? []) {
      ...
      const headers = Object.fromEntries((detail.payload?.headers ?? []).map((h:any)=>[h.name.toLowerCase(),h.value]));
      await db.$executeRaw(Prisma.sql`INSERT INTO inbox_items(user_id,source_provider,source_id,subject,sender,received_at,metadata) VALUES(${connection.user_id},'GOOGLE_GMAIL',${message.id},${headers.subject ?? '(no subject)'},${headers.from ?? null},${headers.date ? new Date(headers.date) : now},${JSON.stringify({threadId:detail.threadId ?? null})}::jsonb) ON CONFLICT DO NOTHING`);
      imported++;
    }
  } catch (e) { errors.push(`gmail: ${(e as Error).message}`); }
...
async function processRun(run:any) {
  ...
  try {
    ...
    await db.$executeRaw(Prisma.sql`UPDATE sync_runs SET status='FAILED', completed_at=now(), error_message=${(e as Error).message} WHERE id=${run.id}`);
  }
}

async function loop() {
  const runs = await db.$queryRaw<any[]>(Prisma.sql`SELECT id,connection_id FROM sync_runs WHERE status='QUEUED' ORDER BY started_at NULLS FIRST, created_at ASC LIMIT 3`);
