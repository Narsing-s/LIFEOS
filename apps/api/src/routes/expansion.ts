...
export async function expansionRoutes(app: FastifyInstance) {
  ...
  app.get('/integrations/google/connect', async r => {
    const state = Buffer.from(JSON.stringify({ userId: r.user.id, nonce: crypto.randomUUID() })).toString('base64url');
  ...
  app.get('/integrations/google/callback', async (r, reply) => {
    ...
    try { state = JSON.parse(Buffer.from(q.state,'base64url').toString('utf8')); } catch { return reply.code(400).send({error:'Invalid OAuth state'}); }
    if (state.userId !== r.user.id) return reply.code(403).send({error:'OAuth state does not belong to this account'});
    const token = await exchangeGoogleCode(q.code);
    await app.prisma.$executeRaw(Prisma.sql`INSERT INTO integration_connections (user_id, provider, status, access_token_encrypted, refresh_token_encrypted, token_expires_at, scopes) VALUES (${r.user.id}, 'GOOGLE', 'CONNECTED', ${token.accessTokenEncrypted}, ${token.refreshTokenEncrypted ?? null}, ${token.expiresAt ?? null}, ${JSON.stringify(token.scopes)}::jsonb) ON CONFLICT (user_id, provider, account_id) DO UPDATE SET access_token_encrypted=EXCLUDED.access_token_encrypted, refresh_token_encrypted=COALESCE(EXCLUDED.refresh_token_encrypted,integration_connections.refresh_token_encrypted), token_expires_at=EXCLUDED.token_expires_at, scopes=EXCLUDED.scopes, status='CONNECTED', updated_at=now()`);
  ...
  app.delete('/integrations/:id', async (r, reply) => {
    const {id}=idParam.parse(r.params); const result=await app.prisma.$executeRaw(Prisma.sql`DELETE FROM integration_connections WHERE id=${id} AND user_id=${r.user.id}`); if(!result) return reply.code(404).send({error:'Integration not found'}); return {ok:true};
  });

  app.get('/timeline', async r => {
    const q=z.object({from:z.string().datetime().optional(),to:z.string().datetime().optional(),limit:z.coerce.number().int().min(1).max(200).default(100)}).parse(r.query);
    return app.prisma.$queryRaw(Prisma.sql`SELECT * FROM timeline_events WHERE user_id=${r.user.id} AND (${q.from ? Prisma.sql`occurred_at >= ${new Date(q.from)}` : Prisma.sql`true`}) AND (${q.to ? Prisma.sql`occurred_at <= ${new Date(q.to)}` : Prisma.sql`true`}) ORDER BY occurred_at DESC LIMIT ${q.limit}`);
  });

  app.post('/timeline', async r => {
    const b=z.object({eventType:z.string().max(60),title:z.string().min(1).max(500),description:z.string().optional(),occurredAt:z.string().datetime(),endAt:z.string().datetime().optional(),sourceProvider:z.string().max(40).optional(),sourceId:z.string().max(255).optional(),location:z.any().optional(),people:z.array(z.any()).default([]),linkedRecords:z.array(z.any()).default([]),metadata:z.any().default({})}).parse(r.body);
    return app.prisma.$queryRaw(Prisma.sql`INSERT INTO timeline_events (user_id,event_type,title,description,occurred_at,end_at,source_provider,source_id,location,people,linked_records,metadata) VALUES (${r.user.id},${b.eventType},${b.title},${b.description??null},${new Date(b.occurredAt)},${b.endAt?new Date(b.endAt):null},${b.sourceProvider??null},${b.sourceId??null},${json(b.location)}::jsonb,${json(b.people)}::jsonb,${json(b.linkedRecords)}::jsonb,${json(b.metadata)}::jsonb) RETURNING *`);
  });
  ...
  app.post('/sync/:provider', async r => { const provider=z.string().min(1).max(40).parse((r.params as {provider:string}).provider.toUpperCase()); const rows=await app.prisma.$queryRaw<{id:string}[]>(Prisma.sql`SELECT id FROM integration_connections WHERE user_id=${r.user.id} AND provider=${provider} AND status='CONNECTED' LIMIT 1`); if(!rows[0]) return {ok:false,status:'NOT_CONNECTED',provider}; const run=await app.prisma.$queryRaw(Prisma.sql`INSERT INTO sync_runs(user_id,connection_id) VALUES(${r.user.id},${rows[0].id}) RETURNING id,status,started_at`); return {ok:true,provider,status:'QUEUED',run}; });
}
