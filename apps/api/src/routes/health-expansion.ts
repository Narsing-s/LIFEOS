...
export async function healthExpansionRoutes(app: FastifyInstance) {
  ...
  const syncSelect = Prisma.sql`SELECT id, connection_id AS "connectionId", status, started_at AS "startedAt", finished_at AS "finishedAt", imported_count AS "importedCount", skipped_count AS "skippedCount", error_count AS "errorCount", error FROM sync_runs WHERE user_id=${(null as unknown as string)}`;
  app.get('/sync/runs', async r => app.prisma.$queryRaw(Prisma.sql`SELECT id, connection_id AS "connectionId", status, started_at AS "startedAt", finished_at AS "finishedAt", imported_count AS "importedCount", skipped_count AS "skippedCount", error_count AS "errorCount", error FROM sync_runs WHERE user_id=${r.user.id} ORDER BY created_at DESC LIMIT 25`));
  app.get('/life/overview', async r => {
    const [timeline,inbox,connections,syncs]=await Promise.all([
      ...
      app.prisma.$queryRaw(Prisma.sql`SELECT id,connection_id AS "connectionId",status,started_at AS "startedAt",finished_at AS "finishedAt",imported_count AS "importedCount",skipped_count AS "skippedCount",error_count AS "errorCount",error FROM sync_runs WHERE user_id=${r.user.id} ORDER BY created_at DESC LIMIT 5`)
    ]);
