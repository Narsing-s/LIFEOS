...
export async function healthExpansionRoutes(app: FastifyInstance) {
  ...
  app.get('/life/overview', async r => {
    const [timeline,inbox,connections,syncs]=await Promise.all([
      ...
      app.prisma.$queryRaw(Prisma.sql`SELECT provider,status,account_email AS "accountEmail",last_synced_at AS "lastSyncedAt" FROM integration_connections WHERE user_id=${r.user.id} ORDER BY created_at DESC`),
